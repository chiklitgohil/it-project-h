...
int validatePassword(const char *pwd)
{
    ...
    for (int i = 0; pwd[i]; ++i) {
        ...
        if (isalpha((unsigned char)pwd[i])) hasAlpha = 1;
    }
...
static void showBreadcrumb(const char *role, int id, const char *section)
{
    ...
    putchar('-'); for (int i=0;i<50;i++) putchar('-'); putchar('\n');
}
...
int patientSignup(void)
{
    ...
    if (strlen(c.username) == 0) { printf("Username cannot be empty.\n"); return 0; }
    ...
    printf("Age: ");
    if (scanf("%d", &p.age) != 1) { clearStdin(); p.age = 0; }
    clearStdin();
    printf("Gender: ");
...
/* Patient portal - guided (asks step-by-step) */
void patientPortal(int patientId)
{
    ...
    while (keepGoing) {
        ...
        if (confirmYesNo("View my profile?")) {
            showPatientProfile(patientId);
            printf("\n");
        }

        if (confirmYesNo("See list of doctors?")) {
            showBreadcrumb("Patient", patientId, "View Doctors");
            viewDoctors();
            printf("\n");
        }

        if (confirmYesNo("Book a new appointment?")) {
            printf("Booking appointment. When prompted for Patient ID, enter your ID: %d\n", patientId);
            scheduleAppointment();
            printf("\n");
        }

        if (confirmYesNo("View my appointments?")) {
            listAppointmentsForPatient(patientId);
            printf("\n");
        }

        if (confirmYesNo("View my medical history?")) {
            printf("When prompted, enter your Patient ID: %d\n", patientId);
            viewPatientMedicalHistory();
            printf("\n");
        }

        if (!confirmYesNo("Do you want to perform another action?")) {
            printf("Logging out...\n");
            keepGoing = 0;
        }
        if (keepGoing) {
...
/* Doctor portal - guided (asks step-by-step) */
void doctorPortal(int doctorId)
{
    ...
    while (keepGoing) {
        ...
        if (confirmYesNo("View all patients?")) {
            showBreadcrumb("Doctor", doctorId, "View Patients");
            viewPatients();
            printf("\n");
        }
        ...
        if (confirmYesNo("Add a medical record?")) {
            printf("Add medical record. When prompted, enter the Appointment ID for which you want to add a record.\n");
            addMedicalRecord();
            printf("\n");
        }

        if (confirmYesNo("Cancel an appointment?")) {
            printf("Cancel appointment. When prompted, enter the Appointment ID to cancel.\n");
            cancelAppointment();
            printf("\n");
        }
...
void authMenu(void)
{
    ...
    do {
        ...
        printf("0) Exit\n");
        printf("Choose: ");
        if (scanf("%d", &choice) != 1) { clearStdin(); choice = -1; }
        clearStdin();
        switch (choice) {
            case 1:
                patientLogin();
